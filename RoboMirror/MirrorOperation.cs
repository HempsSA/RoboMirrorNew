...
namespace RoboMirror
{
	...
	public interface IMirrorOperationStatus
	{
		...
		void OnScanProgress(string side, long fileCount, long directoryCount, long totalSize);
	}
	#endregion
	...
	public sealed class MirrorOperation : IDisposable, ISynchronizeInvoke
	{
		...
		#region Parallel scanning and simulation process

		/// <summary>
		/// Kicks off parallel directory scanning of both source and
		/// destination folders, then starts the Robocopy simulation.
		/// The two scans run on separate threads so that both sides
		/// are enumerated at the same time.
		/// </summary>
		private void StartSimulationProcess()
		{
			_status.OnEnterNewStage("Scanning...", "Source and destination are being scanned in parallel...");
			_status.IsAbortingSupported = false;

			_scanCancellationTokenSource = new CancellationTokenSource();

			var sourceScanner = new DirectoryScanner(SourceFolder);
			var destScanner = new DirectoryScanner(DestinationFolder);

			sourceScanner.ProgressChanged += (s, e) =>
			{
				_status.OnScanProgress("source", e.FileCount, e.DirectoryCount, e.TotalSize);
			};

			destScanner.ProgressChanged += (s, e) =>
			{
				_status.OnScanProgress("destination", e.FileCount, e.DirectoryCount, e.TotalSize);
			};

			var token = _scanCancellationTokenSource.Token;

			// scan both sides on separate threads, then start the
			// Robocopy simulation once both scans have finished
			var sourceTask = new Thread(() => sourceScanner.Scan(token)) { IsBackground = true };
			var destTask = new Thread(() => destScanner.Scan(token)) { IsBackground = true };

			sourceTask.Start();
			destTask.Start();

			// wait for both scans to finish in a background thread
			...
			var waitThread = new Thread(() =>
			{
				...
				int totalErrors = sourceScanner.Result.ErrorCount + destScanner.Result.ErrorCount;
				if (totalErrors > 0)
				{
					SmartEventInvoker.FireEvent(
						new EventHandler((s, e) =>
							_status.OnEnterNewStage("Scanning...",
								string.Format("Scanning complete with {0} access error{1}.",
									totalErrors, totalErrors == 1 ? "" : "s"))),
						this, EventArgs.Empty);
				}

				// transition into the Robocopy simulation
				StartRobocopySimulation();
			}) { IsBackground = true };
		...
		/// <summary>
		/// Starts the actual Robocopy simulation process (/l) after
		/// the parallel directory scanning phase has completed.
		/// </summary>
		private void StartRobocopySimulation()
		{
