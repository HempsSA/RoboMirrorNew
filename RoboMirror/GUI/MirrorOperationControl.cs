...
namespace RoboMirror.GUI
{
	public partial class MirrorOperationControl : UserControl
	{
		...
		private class Status : IMirrorOperationStatus
		{
			...
			public void OnScanProgress(string side, long fileCount, long directoryCount, long totalSize)
			{
				string sizeLabel = FormatSize(totalSize);
				string suffix = string.Format("{0}: {1} files in {2} folders ({3})",
					side, fileCount, directoryCount, sizeLabel);
				_control.statusLabel.Text = string.Format("Scanning {0}...", suffix);
			}
		}
		#endregion
